<template>
  <div style="padding: 60px 0;">
    <b-navbar fixed="top" type="dark" variant="info" style="height: 60px;">
      <b-navbar-nav>
        <b-button v-if="panel==='ConfigMenu'" @click="$emit('goto', 'MyPage')" variant="info" >
    ...
    <ConfigMember v-if="panel==='ConfigMember'" :shareId="shareId" class="config-container"/>
    <ConfigCategory v-if="panel==='ConfigCategory'" :shareId="shareId" class="config-container"/>
    <ConfigNotice v-if="panel==='ConfigNotice'" :shareId="shareId" class="config-container"/>
  </div>
</template>
