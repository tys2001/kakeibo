  ...
  <div v-else>
    <b-list-group>
      ...
      >
        ...
        <div class="sumPayAmount"><font-awesome-icon icon="yen-sign" class="fa-xs"/>{{summary.sumPayAmount}}</div>
        <div class="shareAmount">
          <font-awesome-icon icon="yen-sign" class="fa-xs"/>{{summary.shareAmount + ' ' + summary.shareType}}
          <font-awesome-icon v-if="summary.confirmDatetime" icon="check" style="color: var(--success);"/>
        </div>
        <b-button
  ...
  methods: {
    search() {
      ...
      const params = {
        ...
        year: this.selectedYearMonth.year,
        month: this.selectedYearMonth.month,
      }
      axios.all([
        Utils.httpGet('https://tyspine.com/service/money/showSummaryPay', params),
        Utils.httpGet('https://tyspine.com/service/money/showSummaryByCategory', params),
      ])
      .then(responses => {
